...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def recovered: Observable[Try[T]] = {
      ...
      obs.subscribe(
        x => subject.onNext(Success(x)),
        e => {
          subject.onNext(Failure(e))
          subject.onCompleted()
        },
        () => subject.onCompleted())
    ...
    def timedOut(totalSec: Long): Observable[T] = {
      ...
      val subject = PublishSubject[T](ObservableEx.defaultOf[T])
      obs.subscribe(subject)
      Observable.interval(totalSec seconds).take(1).subscribe(x => subject.onCompleted())
