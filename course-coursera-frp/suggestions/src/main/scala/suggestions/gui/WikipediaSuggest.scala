...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
//    val suggestions: Observable[Try[List[String]]] = searchTerms.map(request => {
//      val response = wikipediaSuggestion(request)
//      Observable(response).map(x => x.v)
//    }).flatten
    val suggestions: Observable[Try[List[String]]] = searchTerms.concatRecovered(x =>
      ObservableEx(wikipediaSuggestion(x)))
    ...
    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.map(_ => suggestionList.selection.items(0).toString)
    ...
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(pageContent) => editorpane.text = pageContent
        case Failure(e) =>
      }
    }
