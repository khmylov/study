...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    while (response.hasNext && !token.isCancelled) {
      println(">>>>>> Can write to response, writing..")
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val handlingSubscription = Future.run()(ct => async {
      while (!ct.isCancelled) {
        println(">>>>>> Waiting for request")
        val (request, exchange) = await { listener.nextRequest() }
        println(">>>>>> Recieved request, processing..")
        respond(exchange, ct, handler(request))
      }
    })

    Subscription(listenerSubscription, handlingSubscription)
  }
...
object NodeScala {
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      ...
      createContext(e => {
        println(">>>>>> Listener recieved request")
        removeContext()
  ...
  object Listener {
    class Default(val port: Int, val relativePath: String) extends Listener {
      ...
      def start() = {
        ...
        new Subscription {
          def unsubscribe() = {
            println(">>>>>> Unsubscribing listener")
            s.stop(0)
            executor.shutdown()
          }
        }
