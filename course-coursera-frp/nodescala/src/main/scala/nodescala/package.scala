...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      p.success(Nil)
      fs.foldRight (p.future) ((f, acc) => for { x <- f; xs <- acc } yield x::xs)
    }
     ...
     */
    def any[T](fs: List[Future[T]]): Future[T] = fs match {
      case Nil => Future { throw new NoSuchElementException() }
      case _ => {
        val p = Promise[T]()
        for (f <- fs)
          f.onComplete(t => p.tryComplete(t))
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      ...
      f onComplete {
        case Success(_) =>
          try {
            p.complete(Success(cont(f.value.get)))
          } catch {
            case e: Throwable => p.failure(e)
          }
        case Failure(e) => p.failure(e)
